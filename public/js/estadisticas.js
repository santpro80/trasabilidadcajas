import { db, collection, query, where, getDocs, doc, getDoc, unSanitizeFieldName } from './firebase-config.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadMonthlyStats = async () => {
        ...
        try {
            ...
            if (querySnapshot.empty) {
                ...
                currentBoxStatsData = []; // Clear data if no results
                return;
            }
    ...
    const showConsumptionForModel = async (modelName) => {
        ...
        try {
            ...
            const sanitizedModel = modelName.replace(/\//g, '_slash_').replace(/\./g, '_dot_').replace(/,/g, '_comma_'); // Re-sanitize modelName for field access
