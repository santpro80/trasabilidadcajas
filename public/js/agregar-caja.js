...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderItemRow = (itemName, isDynamic = false) => {
        ...
        if (isDynamic) formGroup.classList.add('dynamic-item-row');
        let itemHtml;
        if (isDynamic) {
            if (canUseDropdowns) {
                ...
                itemHtml = `
                    <div class="item-details">
                        <label>Nuevo Ítem (Selecciona las opciones)</label>
        ...
        if (isDynamic && canUseDropdowns) {
            formGroup.querySelectorAll('.dynamic-item-selectors select').forEach(select => {
                select.addEventListener('change', () => handleDropdownChange(formGroup));
            });
        }
    };
    ...
    const loadSchemaAndBuildForm = async () => {
        ...
        if (!modelName || !zonaName) { formTitle.textContent = "Error: Faltan datos"; return; }
        ...
        try {
            ...
            if (schemaDocSnap.exists()) {
                ...
                itemsContainer.innerHTML = "<p>Error: No se encontró el esquema.</p>";
    ...
    saveCajaBtn.addEventListener('click', async () => {
        ...
        itemRows.forEach(row => {
            ...
            if (!serialInput || !serialInput.value.trim()) { allFieldsFilled = false; return; }
        ...
        if (!allFieldsFilled) { messageDiv.textContent = "Por favor, rellena el N° de Serie de todos los ítems."; messageDiv.style.color = "red"; return; }
        if (errorInDynamicRow) { messageDiv.textContent = "Completa todas las opciones o campos manuales de los ítems nuevos."; messageDiv.style.color = "red"; return; }
