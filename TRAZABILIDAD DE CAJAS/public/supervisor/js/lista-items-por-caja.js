...
document.addEventListener('DOMContentLoaded', () => {
            ...
            listItem.innerHTML = `
                <div class="flex-1 w-full grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-4 items-center">
                    <div class="flex items-center gap-4">
                        <div class="w-20 h-20 md:w-24 md:h-24 shrink-0 rounded-2xl bg-slate-100 dark:bg-slate-200 flex items-center justify-center overflow-hidden border border-slate-200 dark:border-slate-300 shadow-inner relative transition-transform group-hover:scale-105 cursor-pointer">
                            <span class="material-symbols-outlined text-[32px] md:text-[40px] text-slate-300 dark:text-slate-400 absolute z-0 pointer-events-none">inventory_2</span>
                        ...
                        <span class="text-[13px] md:text-[15px] whitespace-nowrap md:w-auto font-black tracking-widest text-slate-500 uppercase group-hover:text-villalba-blue transition-colors">${itemCode || 'S/C'}</span>
                    </div>
    ...
    const saveNewItem = async (rowElement) => {
        ...
        // Si no se encontró el código con guiones pero sí sin guiones, guardar la versión sin guiones
