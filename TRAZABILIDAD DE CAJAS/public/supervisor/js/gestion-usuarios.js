...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const editAppPedidos = document.getElementById('edit-app-pedidos');
    const editAppTrazabilidad = document.getElementById('edit-app-trazabilidad');
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            if (filtered.length > 0) {
                ...
                filtered.forEach(user => {
                    ...
                    li.className = "p-3 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer border-b border-gray-100 dark:border-slate-800 transition-colors";
                    li.innerHTML = `
                        <div class="font-bold text-gray-800 dark:text-slate-200">${user.name || 'Sin nombre'}</div>
                        <div class="text-xs text-gray-500 dark:text-slate-400">@${user.username || 'N/A'} | ${user.email}</div>
                    `;
    ...
    const selectUser = (user) => {
        ...
        if (editAppPedidos) {
            editAppPedidos.checked = Array.isArray(user.apps) && user.apps.includes('pedidos');
        }
        if (editAppTrazabilidad) {
            editAppTrazabilidad.checked = Array.isArray(user.apps) && user.apps.includes('trazabilidad');
        }
        if (editAppDeposito) {
            editAppDeposito.checked = Array.isArray(user.apps) && user.apps.includes('deposito');
            if (depositoRoleContainer) {
                depositoRoleContainer.style.display = editAppDeposito.checked ? 'block' : 'none';
            }
        }
        if (editRoleDeposito) {
    ...
    if (updateUserBtn) {
        updateUserBtn.addEventListener('click', async () => {
            ...
            let updatedApps = [];
            if (cachedUser) {
                updatedApps = Array.isArray(cachedUser.apps) ? [...cachedUser.apps] : [];
            }
            if (editAppPedidos) {
                if (editAppPedidos.checked && !updatedApps.includes('pedidos')) {
                    updatedApps.push('pedidos');
                } else if (!editAppPedidos.checked && updatedApps.includes('pedidos')) {
                    updatedApps = updatedApps.filter(a => a !== 'pedidos');
                }
            }
            if (editAppTrazabilidad) {
                if (editAppTrazabilidad.checked && !updatedApps.includes('trazabilidad')) {
                    updatedApps.push('trazabilidad');
                } else if (!editAppTrazabilidad.checked && updatedApps.includes('trazabilidad')) {
                    updatedApps = updatedApps.filter(a => a !== 'trazabilidad');
                }
            }
            if (editAppDeposito) {
                if (editAppDeposito.checked && !updatedApps.includes('deposito')) {
                    updatedApps.push('deposito');
                } else if (!editAppDeposito.checked && updatedApps.includes('deposito')) {
                    updatedApps = updatedApps.filter(a => a !== 'deposito');
                }
            }

            const newRoleDeposito = editRoleDeposito ? editRoleDeposito.value : null;
